...
import { PixelParagraph } from "../pixel-paragraph-words";
...
export default function PixelParagraphWordsDemo() {
  ...
  const [wrapperTag, setWrapperTag] = useState<(typeof WRAPPER_TAGS)[number]>("p");
  ...
  return (
    <div className="w-full space-y-8 py-4">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {/* Text */}
        <ControlGroup label="Paragraph Text" className="sm:col-span-2 lg:col-span-3">
          <textarea
            value={text}
        ...
        {/* Pixel Words */}
        <ControlGroup label="Pixel Words (comma-separated)" className="sm:col-span-2 lg:col-span-3">
          <input
            type="text"
        ...
        <ControlGroup label="Wrapper Element">
          <select
            value={wrapperTag}
            onChange={(e) => setWrapperTag(e.target.value as (typeof WRAPPER_TAGS)[number])}
