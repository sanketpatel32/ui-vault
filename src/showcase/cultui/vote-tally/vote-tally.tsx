...
function useVoteTallyItemContext() {
  ...
  if (!context) {
    throw new Error("VoteTally.Item sub-components must be used within VoteTally.Item");
  }
  return context;
}
...
function VoteTallyRoot({
  ...
  const contextValue = useMemo(
    ...
    [votes, votedItems, disabled, vote, unvote, toggleVote, getVoteCount, hasVoted],
  );

  return (
    <VoteTallyContext.Provider value={contextValue}>
      <ul aria-label="Vote tally list" data-disabled={disabled ? true : undefined} {...props}>
        {children}
      </ul>
...
function VoteTallyGroup({ sortBy = "none", children, ...props }: VoteTallyGroupProps) {
...
function VoteTallyTrigger({ children, onClick, ...props }: VoteTallyTriggerProps) {
...
function VoteTallyDescription({ children, ...props }: VoteTallyDescriptionProps) {
  return (
