...
import { PixelHeading } from "../pixel-heading-character";
import type { PixelHeadingMode } from "../pixel-heading-character";
...
export default function PixelHeadingCharacterDemo() {
  ...
  const [prefixFont, setPrefixFont] = useState<(typeof PREFIX_FONTS)[number]>("none");
  const [headingLevel, setHeadingLevel] = useState<(typeof HEADING_LEVELS)[number]>("h1");
    ...
    <div className="w-full space-y-8 py-4">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <ControlGroup label="Heading Level">
          <select
            value={headingLevel}
            onChange={(e) => setHeadingLevel(e.target.value as (typeof HEADING_LEVELS)[number])}
        ...
        <ControlGroup label="Prefix Font">
          <select
            value={prefixFont}
            onChange={(e) => setPrefixFont(e.target.value as (typeof PREFIX_FONTS)[number])}
            className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          >
            {PREFIX_FONTS.map((f) => (
