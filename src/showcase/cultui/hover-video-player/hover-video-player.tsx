...
// Context
const HoverVideoPlayerContext = createContext<HoverVideoPlayerContextType | null>(null);
...
const HoverVideoPlayer: React.FC<HoverVideoPlayerProps> = ({
  ...
  }, [state.isInView, state.isHovering, state.controlsVisible, playVideo, videoSrc]);
  ...
  const contextValue = useMemo<HoverVideoPlayerContextType>(
    () => ({
      ...
      setProgress: (value) => setState((prev) => ({ ...prev, progress: value })),
      setVideoLoading,
      setVideoPlaying,
      cropTop,
      cropBottom,
  ...
  return (
    <HoverVideoPlayerContext.Provider value={contextValue}>
      ...
      >
        ...
        {/* Overlays */}
        {pausedOverlay && (
          <HoverVideoPlayerPausedOverlay>{pausedOverlay}</HoverVideoPlayerPausedOverlay>
        )}
        {loadingOverlay && (
          <HoverVideoPlayerLoadingOverlay>{loadingOverlay}</HoverVideoPlayerLoadingOverlay>
        )}
        {hoverOverlay && !state.isMobile && (
          <HoverVideoPlayerHoverOverlay>{hoverOverlay}</HoverVideoPlayerHoverOverlay>
        )}
...
const HoverVideoPlayerThumbnail: React.FC<{ src: string }> = ({ src }) => {
  ...
  return (
    <motion.div
      className={cn("absolute inset-0 z-1 w-full h-full pointer-events-none transform-gpu")}
      style={{
        top: `-${cropTop}%`,
        bottom: `-${cropBottom}%`,
        height: `calc(100% + ${cropTop + cropBottom}%)`,
 ...
 */
const HoverVideoPlayerControls: React.FC<{ children: React.ReactNode }> = ({ children }) => {
 ...
 */
const HoverVideoPlayerHoverOverlay: React.FC<{ children: React.ReactNode }> = ({ children }) => {
...
const HoverVideoPlayerPiPButton: React.FC = () => {
  ...
  return (
    ...
    >
      {isPiP ? <Minimize className="h-6 w-6" /> : <Maximize className="h-6 w-6" />}
    </Button>
  );
};
