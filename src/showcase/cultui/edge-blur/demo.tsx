import { BottomBlur, EdgeBlur, TopBlur } from "../edge-blur";
...
const SMALL_ROW_KEYS = ["b-1", "b-2", "b-3", "b-4", "b-5", "b-6", "b-7", "b-8"] as const;
  ...
  return (
    <div className="dark:bg-stone-950 flex flex-col items-center gap-10 rounded-md px-4 py-6 md:px-0">
      <div className="max-w-xl space-y-2 text-center">
        <h2 className="text-foreground text-2xl font-bold tracking-tight">Edge blur</h2>
        <p className="text-muted-foreground text-sm">
          Stacked backdrop-blur layers with a gradient mask. Blurs sit <em>outside</em> the scroll
          layer so they stay pinned to the frame; only the inner panel scrolls.{" "}
          <code className="bg-muted rounded px-1 py-0.5 text-xs">transform-gpu</code> on the frame
          keeps <code className="bg-muted rounded px-1 py-0.5 text-xs">fixed</code> blurs inside
          this preview instead of the viewport.
        </p>
      </div>
        ...
        <div className="absolute inset-0 z-0 overflow-y-auto">
          <div className="from-background via-muted/40 to-background min-h-[700px] space-y-3 bg-gradient-to-b p-6 pb-28 pt-8">
            {SCROLL_PARAGRAPH_KEYS.map((key, i) => (
              <p key={key} className="text-foreground/90 text-sm leading-relaxed">
