...
function shapeSvgClassName(flip: ShapeShapeFlip | undefined, className: string | undefined) {
...
function lineArtShapeSvgClassName(flip: ShapeShapeFlip | undefined, className: string | undefined) {
  return cn("w-full fill-none stroke-2", flip ? flipClasses[flip] : null, className);
}
 ...
 */
export function SingleBracketShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  return (
    <svg
    ...
    >
      ...
      <path d="M500 1H6C3 1 1 3 1 6V894C1 897 3 899 6 899H500" stroke="currentColor" />
    </svg>
  );
}
...
export function SeedOfLifeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  return (
    ...
    >
      ...
      {petals.map((p) => (
        <circle cx={p.x} cy={p.y} key={`seed-petal-${p.x}-${p.y}`} r={r} stroke="currentColor" />
      ))}
    </svg>
  );
}
...
export function FlowerOfLifeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  return (
    ...
    >
      ...
      {allCircles.map((p) => (
        <circle cx={p.x} cy={p.y} key={`fol-${p.x}-${p.y}`} r={r} stroke="currentColor" />
      ))}
    </svg>
  );
}
...
export function MetatronsCubeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  return (
    ...
    >
      ...
      {nodes.map((n) => (
        <circle cx={n.x} cy={n.y} key={`mnode-${n.x}-${n.y}`} r={r} stroke="currentColor" />
      ))}
    </svg>
  );
}
...
export function TetrahedronWireshapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r = 280;
...
export function OctahedronWireshapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r = 280;
    ...
    >
      ...
      {eq.map((v) => (
        <g key={`oct-eq-${v.x}-${v.y}`}>
          ...
          <line stroke="currentColor" x1={bottom.x} x2={v.x} y1={bottom.y} y2={v.y} />
        </g>
      ))}
    </svg>
  );
...
export function IcosahedronWireshapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const outerR = 280;
  const innerR = 130;
    ...
    >
      ...
      {outer.map((v, i) => (
        <g key={`ico-pair-${v.x}-${v.y}-${inner[i].x}-${inner[i].y}`}>
          <line stroke="currentColor" x1={v.x} x2={inner[i].x} y1={v.y} y2={inner[i].y} />
          <line
            stroke="currentColor"
            x1={v.x}
            x2={inner[(i + 4) % 5].x}
...
export function DodecahedronWireshapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r1 = 280;
  const r2 = 180;
    ...
    >
      ...
      {outerRing.map((v, i) => (
        <g key={`dode-${v.x}-${v.y}-${midRing[i].x}-${innerRing[i].x}`}>
          <line stroke="currentColor" x1={v.x} x2={midRing[i].x} y1={v.y} y2={midRing[i].y} />
          <line
            stroke="currentColor"
            x1={v.x}
            x2={midRing[(i + 4) % 5].x}
