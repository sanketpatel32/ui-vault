import React, { createContext, forwardRef, type ReactNode, useContext, useState } from "react";
...
const YoutubeVideo = forwardRef<HTMLDivElement, YoutubeVideoProps>(({ videoOpen, url }, ref) => {
  return (
    <AnimatePresence>
      {videoOpen && (
        <motion.div
          ...
          exit="hidden"
        >
          <ReactPlayer width="100%" height="100%" controls={false} color="white" url={url} />
        </motion.div>
      )}
    </AnimatePresence>
  );
...
export default function VideoSectionDemo() {
  ...
  const renderVideoButton = (_handleToggle: () => void) => (
    <div
      className={cn(
    ...
    >
      ...
      <Button className="rounded-r-[33px] py-8 ml-2 " onClick={handleVideoOpen} variant="secondary">
