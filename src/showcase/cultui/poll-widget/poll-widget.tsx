...
import {
  ...
  PopoverTitle,
  PopoverTrigger,
} from "@/showcase/_shared/cultui/ui/popover";
      ...
      >
        ...
        {votedOptions.length > 0 && (
          <p className="text-muted-foreground text-xs">
            You voted for:{" "}
            <span className="font-medium text-foreground">{votedOptions.join(", ")}</span>
          </p>
        )}
...
export function PollWidgetQuestion({ className, children, ...props }: PollWidgetQuestionProps) {
  ...
  return (
    <div
      ...
      {...props}
    >
      <h3 className="font-semibold text-base tracking-tight">{children ?? question}</h3>
