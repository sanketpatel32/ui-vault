...
/**
 * `next/image`-compatible prop surface rendered as a plain `<img>`.
 * Keeps `fill` / `sizes` / `priority` / `unoptimized` accepted (they are
 * stripped at render time) and allows `StaticImageData`-shaped `src` values.
 */
...
function toImgProps({
  ...
  // `next/image`-only flags have no effect on a plain <img> — drop them.
  void fill;
  void sizes;
  void priority;
  void unoptimized;
  ...
  return {
    ...rest,
    src: typeof src === "string" || src === undefined ? src : src.src,
  };
}
...
function revealMaskImage(direction: DitherRevealDirection, from: number, to: number): string {
 ...
 */
const DitherImageOverlay = forwardRef<HTMLImageElement, DitherImageOverlayProps>(
  function DitherImageOverlay(
    { className, direction = "r", from = 0, to = 65, maskClassName, style, ...props },
    ref,
  ) {
    const typedMaskStyle = maskClassName === undefined ? revealMaskStyle(direction, from, to) : {};
