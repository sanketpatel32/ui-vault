...
function DemoSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="w-full space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-3">{children}</div>
    </section>
  );
}
  ...
  return (
    <div className="dark:bg-stone-950 flex flex-col items-center gap-12 rounded-md px-4 py-6 md:px-0">
      <div className="max-w-2xl space-y-4 text-center">
        <h2 className="text-foreground text-2xl font-bold tracking-tight">SVG shapes (animated)</h2>
        <p className="text-muted-foreground text-sm">
          Same wide viewBox geometry as the static set, with paths drawn via{" "}
          <code className="bg-muted rounded px-1 py-0.5 text-xs">motion/react</code> when each
          graphic scrolls into view. Respects{" "}
          <code className="bg-muted rounded px-1 py-0.5 text-xs">prefers-reduced-motion</code>.
        </p>
      ...
      <div key={replayKey} className="flex w-full max-w-5xl flex-col gap-10">
        ...
        <section className="w-full space-y-4">
          <h3 className="text-foreground text-sm font-semibold tracking-tight">Flip prop</h3>
