...
function frameSvgClassName(flip: ShapeShapeFlip | undefined, className: string | undefined) {
  return cn(shapeSvgClass, flip ? flipClasses[flip] : null, className);
}

/** Line-art frames: no fill tint (stroke-only reads correctly). */
function lineArtShapeSvgClassName(flip: ShapeShapeFlip | undefined, className: string | undefined) {
  return cn(
...
function strokeDrawProps(reduceMotion: boolean | null, staggerIndex: number, duration = 0.72) {
  return {
 ...
 */
export function RoundedTriangleShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function ParallelogramShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function RibbonBannerShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function HourglassBowtiePanelSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function ScallopedTopPanelSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function RoundedRectCornerBiteShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function DoubleNotchTechShapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function OgeeOnionDomePanelSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
 ...
 */
export function InvertedArchPanelSvg({ className, flip = "none" }: ShapeSvgProps) {
  const reduceMotion = useReducedMotion();
    ...
    >
      ...
      {nodes.map((n, i) => (
        <motion.circle
          ...
          r={r}
          stroke="currentColor"
          transition={sacredStrokeTransition(reduceMotion, lines.length + i, 0.55)}
          viewport={SACRED_IN_VIEW}
          whileInView={{ pathLength: 1 }}
        />
      ))}
...
export function TetrahedronWireframeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r = 280;
...
export function OctahedronWireframeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r = 280;
...
export function IcosahedronWireframeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const outerR = 280;
  const innerR = 130;
...
export function DodecahedronWireframeSvg({ className, flip = "none" }: ShapeSvgProps) {
  const cx = 1200;
  const cy = 500;
  const r1 = 280;
  const r2 = 180;
  ...
  for (let i = 0; i < 9; i++) {
    ...
    switch (dir) {
      case 0: // cut square from right
        arcs.push(`M${rx + rw},${ry} A${side},${side} 0 0,1 ${rx + rw - side},${ry + side}`);
        rw -= side;
        break;
      case 1: // cut from bottom
        arcs.push(`M${rx + rw},${ry + rh} A${side},${side} 0 0,1 ${rx},${ry + rh - side}`);
        rh -= side;
        break;
    ...
    >
      ...
      {circles.map((c) => (
        <motion.circle
          ...
          transition={sacredStrokeTransition(reduceMotion, torusStagger++, 0.55)}
          viewport={SACRED_IN_VIEW}
          whileInView={{ pathLength: 1 }}
        />
      ))}
