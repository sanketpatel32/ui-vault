...
import { CheckmarkSquare02Icon, PlusSignIcon, SettingsIcon } from "@hugeicons/core-free-icons";
...
import {
  ...
  CommandList,
  CommandSeparator,
} from "@/showcase/_shared/cultui/ui/command";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
} from "@/showcase/_shared/cultui/ui/dialog";
import {
  HoverCard,
  HoverCardContent,
  HoverCardTrigger,
} from "@/showcase/_shared/cultui/ui/hover-card";
import { Input } from "@/showcase/_shared/cultui/ui/input";
import { Popover, PopoverContent, PopoverTrigger } from "@/showcase/_shared/cultui/ui/popover";
...
export const useInstructions = () => {
  ...
  if (!ctx) {
    throw new Error("useInstructions must be used within an Instructions provider");
  }
  return ctx;
};
...
export function Instructions({
  ...
  const [activeIds, setActiveIds] = useControllableState(value, defaultValue, onValueChange);

  const [internalInstructions, setInternalInstructions] = useState<Instruction[]>(
  ...
  const removeCustom = useCallback(
    (id: string) => {
      ...
      if (controlledInstructions) {
        onInstructionsChange?.(controlledInstructions.filter((i) => i.id !== id));
      } else {
...
export function InstructionsTrigger({
  ...
  return (
    <PopoverTrigger asChild>
      <Button
        className={cn("gap-1.5", activeCount > 0 && "text-foreground", className)}
        size="sm"
        type="button"
        variant="ghost"
        {...props}
      >
        {children ?? (
          <>
            <HugeiconsIcon className="size-3.5" icon={SettingsIcon} strokeWidth={2} />
            {label ?? "Instructions"}
            {activeCount > 0 && (
              <span
 ...
 */
export function InstructionsContent({ className, children, ...props }: InstructionsContentProps) {
  return (
    <PopoverContent
      align="start"
 ...
 */
export function InstructionsList({ className, children, ...props }: InstructionsListProps) {
  return (
    <CommandList className={cn("max-h-64", className)} data-slot="instructions-list" {...props}>
      {children}
    </CommandList>
  );
}
...
export function InstructionsEmpty({
  ...
  return (
    <CommandEmpty className={cn(className)} data-slot="instructions-empty" {...props}>
      {children}
    </CommandEmpty>
  );
}
 ...
 */
export function InstructionsGroup({ className, ...props }: InstructionsGroupProps) {
  return (
    <CommandGroup
      className={cn(
 ...
 */
export function InstructionsSeparator({ className, ...props }: InstructionsSeparatorProps) {
  return (
    <CommandSeparator className={cn(className)} data-slot="instructions-separator" {...props} />
  );
}
...
export function InstructionsItem({
  ...
  const itemContent = (
    ...
    >
      ...
      >
        {active && (
          <HugeiconsIcon className="size-3" icon={CheckmarkSquare02Icon} strokeWidth={2.5} />
        )}
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        {children ?? (
          <>
            ...
            <InstructionsItemDescription>{instruction.description}</InstructionsItemDescription>
          </>
        )}
      </div>
    </CommandItem>
  ...
  return <InstructionsHoverCard instruction={instruction}>{itemContent}</InstructionsHoverCard>;
}
 ...
 */
export function InstructionsItemTitle({ className, ...props }: InstructionsItemTitleProps) {
  return (
    <span
 ...
 */
export function InstructionsHoverCard({ instruction, children }: InstructionsHoverCardProps) {
  return (
      ...
      >
        <div className="flex flex-col gap-2">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-muted-foreground text-xs">{instruction.description}</span>
          </div>
          {instruction.content && (
          ...
          {instruction.isCustom && (
            <span className="text-[0.625rem] text-muted-foreground">Custom instruction</span>
          )}
        </div>
      </HoverCardContent>
    </HoverCard>
 ...
 */
export function InstructionsFooter({ className, children, ...props }: InstructionsFooterProps) {
  return (
    <div className={cn("border-t p-1", className)} data-slot="instructions-footer" {...props}>
      {children}
    </div>
  );
}
...
export function InstructionsCreateTrigger({
  ...
  return (
    <CommandItem
      className={cn("flex cursor-pointer items-center gap-2 text-muted-foreground", className)}
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {children ?? (
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-2">
              <label className="font-medium text-xs" htmlFor="instruction-title">
                Title
              </label>
              <Input
            ...
            <div className="flex flex-col gap-2">
              <label className="font-medium text-xs" htmlFor="instruction-description">
                Description
              </label>
              <Input
            ...
            <div className="flex flex-col gap-2">
              <label className="font-medium text-xs" htmlFor="instruction-content">
