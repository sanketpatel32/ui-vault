...
import type { Instruction } from "./ai-instructions";
...
export default function AiInstructionsDemo() {
  const [activeInstructions, setActiveInstructions] = useState<string[]>(["concise"]);
  const [instructions, setInstructions] = useState<Instruction[]>(SAMPLE_INSTRUCTIONS);

  return (
    <div className="flex w-full max-w-md flex-col gap-4">
      <div className="rounded-lg border bg-card p-4">
        <p className="mb-3 text-muted-foreground text-sm">
          Configure AI behavior with custom instructions. Click the button below to manage your
          active instructions.
        </p>
        <Instructions
            ...
            <InstructionsList>
              ...
              <InstructionsGroup heading="Available Instructions">
                {instructions.map((instruction) => (
                  <InstructionsItem instruction={instruction} key={instruction.id} />
                ))}
              </InstructionsGroup>
            </InstructionsList>
            <InstructionsFooter>
      ...
      {activeInstructions.length > 0 && (
        <div className="rounded-lg border bg-muted/50 p-3">
          <p className="mb-2 font-medium text-xs">Active ({activeInstructions.length}):</p>
