...
function FeaturePollBasicExample() {
  ...
  return (
    <div className="w-full max-w-lg">
      <FeaturePoll.Root
        onValueChange={(value) => setSelected(Array.isArray(value) ? (value[0] ?? "") : value)}
        value={selected}
      >
        <FeaturePoll.Header>
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        ...
        <CardDescription>Results appear after you submit your vote.</CardDescription>
      </CardHeader>
      <CardContent>
        <FeaturePoll.Root
          hasVoted={hasVoted}
        ...
        >
          ...
          {!hasVoted && (
            <div className="pt-2">
              <Button className="w-full" disabled={!selected} onClick={() => setHasVoted(true)}>
                Submit Vote
              </Button>
            </div>
          )}
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        ...
        <CardDescription>Select the features you want us to focus on next.</CardDescription>
      </CardHeader>
      <CardContent>
        <FeaturePoll.Root
          hasVoted={hasVoted}
        ...
        >
          <FeaturePoll.Options>
            {features.map((feature) => (
              <FeaturePoll.Option
                disabled={!hasVoted && selected.length >= 2 && !selected.includes(feature.id)}
                key={feature.id}
                value={feature.id}
              >
                <FeaturePoll.Indicator />
