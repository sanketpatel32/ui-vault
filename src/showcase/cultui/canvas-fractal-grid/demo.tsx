...
import {
  ...
  AccordionItem,
  AccordionTrigger,
} from "@/showcase/_shared/cultui/ui/accordion";
import { Button } from "@/showcase/_shared/cultui/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/showcase/_shared/cultui/ui/card";
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@/showcase/_shared/cultui/ui/select";
import { Slider } from "@/showcase/_shared/cultui/ui/slider";
import { Switch } from "@/showcase/_shared/cultui/ui/switch";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/showcase/_shared/cultui/ui/tabs";
import { useToast } from "@/showcase/_shared/cultui/ui/use-toast";

import { CanvasFractalGrid } from "./canvas-fractal-grid";
...
`;
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">Configurable CanvasFractalGrid</h1>
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-8">
        <Button variant="outline" size="sm" onClick={copyConfigToClipboard} className="ml-2">
              ...
              <TabsContent value="general">
                <Accordion type="single" collapsible className="w-full">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <Slider
                            ...
                            onValueChange={([value]) => updateConfig("waveIntensity", value)}
                          />
                        </div>
                        <div>
                          ...
                          <Slider
                            ...
                            onValueChange={([value]) => updateConfig("waveRadius", value)}
                          />
                        </div>
                      </div>
                    </AccordionContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={(checked) => updateConfig("enableNoise", checked)}
                          />
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={(checked) => updateConfig("enableMouseGlow", checked)}
                          />
                  ...
                  <AccordionItem value="performance">
                    ...
                    <AccordionContent>
                      <div>
                        <Label htmlFor="initialPerformance">Initial Performance</Label>
                        <Select
                          value={config.initialPerformance}
                          onValueChange={(value) =>
                            updateConfig("initialPerformance", value as "low" | "medium" | "high")
                          }
                        >
              ...
              <TabsContent value="gradients">
                <div className="space-y-4">
                  {config.gradients.map((gradient, index) => (
                    <Card key={index} className="bg-gradient-to-b from-primary/5 to-primary/10">
                      <CardHeader>
                        <CardTitle className="text-lg">Gradient {index + 1}</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <Label htmlFor={`centerX-${index}`}>Center X</Label>
                              <Slider
                                id={`centerX-${index}`}
                                min={0}
                                max={100}
                            ...
                            <div>
                              <Label htmlFor={`centerY-${index}`}>Center Y</Label>
                              <Slider
                                id={`centerY-${index}`}
                                min={0}
                                max={100}
                          ...
                          {gradient.stops.map((stop, stopIndex) => (
                            <div key={stopIndex} className="flex items-center space-x-2">
                              ...
                              <Button
                                ...
                                size="icon"
                                onClick={() => {
                                  const newStops = gradient.stops.filter((_, i) => i !== stopIndex);
                                  updateGradient(index, {
                                    ...gradient,
                                    stops: newStops,
                                  });
                          ...
                          <div className="flex justify-between">
                            ...
                            <Button variant="destructive" onClick={() => removeGradient(index)}>
