...
export function castleWallBandPath(toothCount: number): string {
  ...
  for (let i = 0; i < toothCount; i++) {
    const xMerlonEnd = CASTLE_WALL_X_MAX - ((2 * i + 1) * span) / (2 * toothCount);
    const xCreneEnd = CASTLE_WALL_X_MAX - ((2 * i + 2) * span) / (2 * toothCount);
    parts.push(
...
export function DiagonalSlashBandSvg({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <path d="M1 249L394 8C397 4 400 2 404 2H2399V249H1Z" stroke="currentColor" />
    </svg>
  );
}
 ...
 */
export function SteppedLeftSteepRightBandSvg({ className }: { className?: string }) {
  return (
    <svg
      className={cn(shapeSvgClass, className)}
      fill="none"
...
export function SteppedInsetFrameSvg({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <path d="M1 1H2399V300H2350V700H2399V999H1V700H50V300H1Z" stroke="currentColor" />
    </svg>
  );
}
 ...
 * Closed band — filled lid + stroke — aligned to 2400-wide ornaments.
 */
export function PyramidStepBandLotsOfStepsSvg({ className }: { className?: string }) {
  return (
    <svg
      className={cn(shapeSvgClass, className)}
      fill="none"
