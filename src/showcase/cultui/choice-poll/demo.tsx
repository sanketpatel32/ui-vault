...
import { ChoicePoll } from "./choice-poll";
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        ...
        <CardDescription>Help us prioritize which integration to build next</CardDescription>
      </CardHeader>
      <CardContent>
        <ChoicePoll.Root
          hasVoted={hasVoted}
        ...
        >
          <ChoicePoll.Options>
            {integrations.slice(0, 4).map((integration) => (
              <ChoicePoll.Option key={integration.id} value={integration.id}>
                ...
                <HugeiconsIcon className="h-5 w-5 text-muted-foreground" icon={integration.icon} />
          ...
          {!hasVoted && (
            <div className="pt-2">
              <Button className="w-full" disabled={!selected} onClick={handleVote}>
                Submit Vote
              </Button>
            </div>
          )}
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        ...
        <CardDescription>Select up to 3 integrations you'd like us to prioritize</CardDescription>
      </CardHeader>
      <CardContent>
        <ChoicePoll.Root
          hasVoted={hasVoted}
        ...
        >
          <ChoicePoll.Options>
            {integrations.map((integration) => (
              <ChoicePoll.Option
                disabled={!hasVoted && selected.length >= 3 && !selected.includes(integration.id)}
                key={integration.id}
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-0.5">
                  <ChoicePoll.Label>{integration.label}</ChoicePoll.Label>
                  <span className="text-muted-foreground text-xs">{integration.description}</span>
                </div>
                <ChoicePoll.Percentage />
              </ChoicePoll.Option>
            ))}
          ...
          {!hasVoted && (
            <div className="pt-2">
              <Button className="w-full" disabled={selected.length === 0} onClick={handleVote}>
