...
import { MetalButton, MetalIconButton } from "../metal-button";
  ...
  return (
    <section className="space-y-4">
      <div className="space-y-1">
        ...
        {description ? (
          <p className="text-pretty text-muted-foreground text-xs leading-relaxed">{description}</p>
        ) : null}
      </div>
      {children}
    </section>
...
export default function MetalButtonDemo() {
  ...
  const activePresetLabel = PRESET_ROW.find((row) => row.key === preset)?.label ?? "Chromatic";
    ...
    <div className="mx-auto w-full max-w-3xl space-y-10 px-4 py-8 md:px-6">
      <header className="space-y-2 text-center">
        ...
        <p className="mx-auto max-w-lg text-pretty text-muted-foreground text-sm leading-relaxed">
          <span className="text-foreground/90">className</span> targets the shadcn{" "}
          <span className="text-foreground/90">Button</span>;{" "}
          <span className="text-foreground/90">metalFxClassName</span> styles the MetalFx wrapper.
          Use <span className="text-foreground/90">preset</span>,{" "}
      ...
      >
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex w-full justify-center rounded-xl bg-neutral-950 px-6 py-8">
            <MetalButton preset={preset} theme="dark" type="button" variant="default">
              {activePresetLabel}
            </MetalButton>
          </div>
        </div>
      ...
      >
        ...
        <p
          className={cn(
            "text-center text-xs",
            respectsReducedMotion ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground",
          )}
          id={`${id}-hint`}
        >
