...
import { useOptionalPromptInputController } from "@/showcase/_shared/cultui/ui/prompt-input";
...
export const usePromptLibrary = () => {
  ...
  if (!ctx) {
    throw new Error("usePromptLibrary must be used within a PromptLibrary provider");
  }
  return ctx;
};
...
export function PromptLibrary({
  ...
  const [internalPrompts, setInternalPrompts] = useState<Prompt[]>(controlledPrompts ?? []);
  ...
  return (
    <PopoverTrigger asChild>
      ...
      >
        {children ?? (
          <>
            <HugeiconsIcon className="size-3.5" icon={LibraryIcon} strokeWidth={2} />
            {label ?? "Prompts"}
          </>
        )}
      </Button>
 ...
 */
export function PromptLibraryContent({ className, children, ...props }: PromptLibraryContentProps) {
  return (
    <PopoverContent
      align="start"
 ...
 */
export function PromptLibraryList({ className, children, ...props }: PromptLibraryListProps) {
  return (
    <CommandList className={cn("max-h-64", className)} data-slot="prompt-library-list" {...props}>
      {children}
    </CommandList>
  );
}
...
export function PromptLibraryEmpty({
  ...
  return (
    <CommandEmpty className={cn(className)} data-slot="prompt-library-empty" {...props}>
      {children}
    </CommandEmpty>
  );
}
 ...
 */
export function PromptLibraryGroup({ className, ...props }: PromptLibraryGroupProps) {
  return (
    <CommandGroup
      className={cn(
...
export type PromptLibrarySeparatorProps = ComponentProps<typeof CommandSeparator>;

/**
 * Visual separator between prompt groups.
 */
export function PromptLibrarySeparator({ className, ...props }: PromptLibrarySeparatorProps) {
  return (
    <CommandSeparator className={cn(className)} data-slot="prompt-library-separator" {...props} />
  );
}
...
export function PromptLibraryItem({
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        {children ?? (
          <>
            ...
            <PromptLibraryItemDescription>{prompt.description}</PromptLibraryItemDescription>
          </>
        )}
      </div>
    </CommandItem>
  ...
  return <PromptLibraryHoverCard prompt={prompt}>{itemContent}</PromptLibraryHoverCard>;
}
 ...
 */
export function PromptLibraryItemTitle({ className, ...props }: PromptLibraryItemTitleProps) {
  return (
    <span
 ...
 */
export function PromptLibraryHoverCard({ prompt, children }: PromptLibraryHoverCardProps) {
  return (
    <HoverCard openDelay={300} closeDelay={100}>
      ...
      >
        <div className="flex flex-col gap-2">
          ...
          {prompt.isCustom && (
            <span className="text-[0.625rem] text-muted-foreground">Custom prompt</span>
          )}
        </div>
      </HoverCardContent>
    </HoverCard>
 ...
 */
export function PromptLibraryFooter({ className, children, ...props }: PromptLibraryFooterProps) {
  return (
    <div className={cn("border-t p-1", className)} data-slot="prompt-library-footer" {...props}>
      {children}
    </div>
  );
}
...
export function PromptLibraryCreateDialog({
  ...
  const { createDialogOpen, setCreateDialogOpen, addCustom } = usePromptLibrary();
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {children ?? (
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-2">
              <label className="font-medium text-xs" htmlFor="prompt-description">
                Description
              </label>
              <Input
