    ...
    <div className="flex w-full max-w-md flex-col gap-4">
      <div className="rounded-lg border bg-card p-4">
        <p className="mb-3 text-muted-foreground text-sm">
          Browse and insert prompt templates. Click a prompt to copy it to clipboard (or insert
          directly when used with PromptInput).
        </p>
        <PromptLibrary onPromptsChange={setPrompts} onSelect={setLastSelected} prompts={prompts}>
          <PromptLibraryTrigger />
          <PromptLibraryContent>
            <PromptLibrarySearch />
            <PromptLibraryList>
      ...
      {lastSelected && (
        <div className="rounded-lg border bg-muted/50 p-3">
          ...
          <p className="mt-1 line-clamp-2 text-muted-foreground text-xs">{lastSelected.prompt}</p>
        </div>
      )}
    </div>
  );
