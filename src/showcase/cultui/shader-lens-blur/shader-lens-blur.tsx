...
export function ShaderLensBlur() {
  ...
  const updateSize = useCallback(() => {
    ...
    const drawingBufferSize = rendererRef.current.getDrawingBufferSize(new THREE.Vector2());
  ...
  const updateMousePosition = useCallback(
    (x: number, y: number) => {
      ...
      if (isInteracting || config.enableHover) {
        materialRef.current.uniforms.u_mouse.value.set(mouseX, rect.height - mouseY);
      }
    },
  ...
  return (
    ...
    >
      ...
      >
        {config.enableHover
          ? `${config.invertMouse ? "Inverted mouse" : "Normal mouse"} interaction`
